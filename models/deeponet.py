    ...
    def __call__(self, branch_x, trunk_x):
        ...
        # otherwise, we have a single branch
            ...
            for i, fs in enumerate(self.branch_layers[:-1]):
                ...
                if i == 1:
                    branch_skip1 = branch_x
                if i == 2:  # 3-й слой (индексация с 0)
                    branch_skip2 = branch_x
                branch_x = nn.activation.tanh(branch_x)
                branch_x = nn.Dropout(rate=self.dropout_rate)(branch_x, deterministic=not self.is_training)
                if i == 3 and branch_skip1 is not None:
                    if branch_skip1.shape[-1] != branch_x.shape[-1]:
                        branch_skip1 = nn.Dense(branch_x.shape[-1], name="branch_skip_proj1")(branch_skip1)
                    branch_x = branch_x + branch_skip1
                if i == 4 and branch_skip2 is not None:  # 5-й слой
                    # Если размерности не совпадают, добавляем проекцию
                    if branch_skip2.shape[-1] != branch_x.shape[-1]:
                        branch_skip2 = nn.Dense(branch_x.shape[-1], name="branch_skip_proj2")(branch_skip2)
                    branch_x = branch_x + branch_skip2
        ...
        for i, fs in enumerate(self.trunk_layers):
            ...
            trunk_x = nn.Dropout(rate=self.dropout_rate)(trunk_x, deterministic=not self.is_training)
            if i == 3 and trunk_skip is not None:
                if trunk_skip.shape[-1] != trunk_x.shape[-1]:
                    trunk_skip = nn.Dense(trunk_x.shape[-1], name="trunk_skip_proj")(trunk_skip)
                trunk_x = trunk_x + trunk_skip
        # reshape the output
        ...
        if self.stacked:
            ...
            for j in range(self.branch_layers[-1]):
                ...
                for i, fs in enumerate(self.branch_layers[:-1]):
                    ...
                    branch_x_j = nn.Dropout(rate=self.dropout_rate)(branch_x, deterministic=not self.is_training)
